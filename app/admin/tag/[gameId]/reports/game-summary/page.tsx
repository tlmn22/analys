...
import { computeGameSummary, elapsedSeconds } from "./summary-stats";
import { GameSummaryClient } from "./game-summary-client";
...
export default async function GameSummaryPage({
  ...
  const [eventsRes, seasonTeamsRes] = await Promise.all([
    db
      .from("game_events")
      .select(
        "id, video_time, period, clock_time, event_type, team_id, player_id, assist_player_id, points, shot_type, shot_x, shot_y, and_one"
      )
      .eq("game_id", gameId),
    db
      .from("season_teams")
  ...
  const summary = computeGameSummary(
    ...
    info.game.home_team_id,
    info.game.visitor_team_id
  );
  ...
  const rawEvents = rows.map((e) => ({
    ...
    points: e.points as number | null,
    shotType: e.shot_type as string | null,
    shotX: e.shot_x as number | null,
    shotY: e.shot_y as number | null,
    andOne: !!e.and_one,
  }));
      ...
      <div className="mx-auto max-w-[1600px]">
        ...
        <div className="mt-8">
          <GameSummaryClient
            ...
            finalHome={summary.finalHome}
            finalVisitor={summary.finalVisitor}
