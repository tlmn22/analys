...
import { HustleContactClient } from "./hustle-contact-client";

export const dynamic = "force-dynamic";

export default async function HustleContactPage({
  params,
}: {
  ...
  const [eventsRes, seasonTeamsRes] = await Promise.all([
    db
      .from("game_events")
      .select(
        "id, video_time, period, clock_time, event_type, team_id, player_id, hustle_play_type, physical_contact_type, physical_contact_second_player_id, physical_contact_winner_player_id"
      )
      .eq("game_id", gameId),
    db
      .from("season_teams")
  ...
  const rawEvents = rows.map((e) => ({
    ...
    shotY: null,
    andOne: false,
    hustlePlayType: e.hustle_play_type as string | null,
    physicalContactType: e.physical_contact_type as string | null,
    physicalContactSecondPlayerId: e.physical_contact_second_player_id as string | null,
    physicalContactWinnerPlayerId: e.physical_contact_winner_player_id as string | null,
  }));
      ...
      <div className="mx-auto max-w-[1600px]">
        ...
        <div className="mt-3">
          <ReportHeader
            ...
            title="Hustle & Contact"
            activeSlug="hustle-contact"
            info={info}
          />
        </div>
