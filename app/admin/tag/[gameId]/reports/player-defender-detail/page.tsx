...
import { PlayerDefenderDetailClient } from "./player-defender-detail-client";

export const dynamic = "force-dynamic";

export default async function PlayerDefenderDetailPage({
  params,
}: {
  ...
  const [eventsRes, seasonTeamsRes] = await Promise.all([
    db
      .from("game_events")
      .select(
        "id, video_time, period, clock_time, event_type, team_id, player_id, points, defender_player_id, contested_close, lightly_contested, uncontested, wide_open"
      )
      .eq("game_id", gameId),
    db
      .from("season_teams")
  ...
  const rawEvents = rows.map((e) => ({
    ...
    shotY: null,
    andOne: false,
    defenderPlayerId: e.defender_player_id as string | null,
    contestedClose: !!e.contested_close,
    lightlyContested: !!e.lightly_contested,
    uncontested: !!e.uncontested,
    wideOpen: !!e.wide_open,
  }));
      ...
      <div className="mx-auto max-w-[1600px]">
        ...
        <div className="mt-3">
          <ReportHeader
            ...
            title="Player Defender Detail"
            activeSlug="player-defender-detail"
            info={info}
          />
        </div>

        <div className="mt-8">
          <PlayerDefenderDetailClient
